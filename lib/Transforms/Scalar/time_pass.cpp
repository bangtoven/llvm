...
namespace {
  struct TimeMeasurePass : public FunctionPass {
    ...
    */
    ...
    void handleLoop(ScalarEvolution &SE, Loop *L, int& loopcounter, Function &F){
      ...
      string loop_id = m_name+f_name+ToString(loopcounter);
      char *loop_id_cstr = new char[loop_id.length() + 1];
      strcpy(loop_id_cstr, loop_id.c_str());
      
      // compute hash for string
      unsigned long loop_id_hash = hash(loop_id_cstr);
      ...
      */
      ...
      {
        /*
          In main, if we have an outer loop OR
          if we have a second level loop with trip_count < 30,
          include in our analysis.
        */
        if ((L->getLoopDepth() == outer_depth) ||
              (((L->getLoopDepth()  - outer_depth) == 1) &&
                  ((trip_count > 0 && trip_count < mL2TCCO))
              )
            )
        {
          writeFeatures(loop_id_hash, L, trip_count);
          runOnEntryBlock(Preheader, loop_id_hash, F);
          for (LoopInfo::iterator LIT = L->getSubLoops().begin(), LEND = L->getSubLoops().end(); LIT != LEND; ++LIT) {
            handleLoop(SE, *LIT, loopcounter, F);
          }
          runOnExitBlock(exitingBlock, loop_id_hash, F);
        }
      }
      else {
        /*
          For other functions, use outer loops with trip_counts up to 100 OR
          Use second_level loops with trip_count < 10
        */
        if (((L->getLoopDepth() == outer_depth) && (trip_count > 0 && trip_count < oL1TCCO)) ||
              (((L->getLoopDepth()  - outer_depth) == 1) &&  (trip_count > 0 && trip_count < oL2TCCO))
            )
        {
          writeFeatures(loop_id_hash, L, trip_count);
          runOnEntryBlock(Preheader, loop_id_hash, F);
    ...
    virtual bool runOnFunction (Function &F) {
      ...
      FunctionType *finalFuncType = FunctionType::get(retType, false);
      ...
      if (!F.isDeclaration()) {
        ...
        if (F.getName() == "main"){
          Function::iterator bit = F.end();
            bit--;
          Instruction *newInst = CallInst::Create(hookFuncFinal, "");
    ...
    void runOnEntryBlock(BasicBlock* Preheader, unsigned long hash, Function &F)
    {
      LLVMContext &Ctx = F.getContext();
      llvm::Value* arg []= {llvm::ConstantInt::get(Ctx , llvm::APInt( 64,hash ))};
      Instruction *newInst = CallInst::Create(hookFuncRecordStart, arg, "");
      newInst->insertBefore(Preheader->begin());
    }

    void runOnExitBlock(BasicBlock* exitingBlock, unsigned long hash, Function &F)
    {
      LLVMContext &Ctx = F.getContext();
      llvm::Value* arg []= {llvm::ConstantInt::get(Ctx , llvm::APInt( 64,hash ))};
